...
import { BOARD_LAYOUT, COLORS, ICONS } from '../constants';
...
const Board: React.FC<BoardProps> = ({ players, activeCardType, onDeckClick, onTileClick }) => {
  ...
  const getTileStyles = (index: number) => {
    ...
    const pos = offsets[index] || { x: index % 6, y: index };
    return {
      left: `${pos.x * 160 + 100}px`,
      top: `${pos.y * 130 + 350}px`,
    };
  };
  ...
  const getPathData = () => {
    ...
    BOARD_LAYOUT.forEach((_, i) => {
      const style = getTileStyles(i);
      const x = parseInt(style.left) + 45;
      const y = parseInt(style.top) + 45;
      if (i === 0) d += `${x} ${y}`;
      else {
          const prev = getTileStyles(i - 1);
          const px = parseInt(prev.left) + 45;
          const py = parseInt(prev.top) + 45;
          const midX = (px + x) / 2;
          const midY = (py + y) / 2;
  ...
  return (
    <div 
        ref={containerRef}
        className="relative origin-top-left transition-transform duration-700 min-w-[1300px] min-h-[4150px] overflow-visible pb-40 pt-80"
    ...
    >
      ...
      {BOARD_LAYOUT.map((tile, idx) => {
        const styles = getTileStyles(idx);
